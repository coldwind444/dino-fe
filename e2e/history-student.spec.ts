...
test.describe('History View - Student Role', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForURL(/\/student\/history/, { waitUntil: 'commit', timeout: 15000 });
    await expect(page).toHaveURL(/\/student\/history/);
  });

  test('TC-08A-01: Page load and initial data', async ({ page }) => {
  ...
  test('TC-08A-02: Filter - filter by category', async ({ page }) => {
  ...
  test('TC-08A-03: Filter - keyword', async ({ page }) => {
    // Set category filter and apply
    const filterBtn = page.getByTestId('filter-btn')
    const totalExercises = page.getByTestId('total-exercises');
    const accuracy = page.getByTestId('accuracy');
  ...
  test('TC-08A-04: Filter - Valid Date', async ({ page }) => {
    // Set category filter and apply
    const filterBtn = page.getByTestId('filter-btn')
    ...
    await startDateInput.fill('01/05/25');
  ...
  test('TC-08A-05: Filter - Invalid Date (start > end)', async ({ page }) => {
    const startDateInput = page.getByTestId('start-date-input');
    const endDateInput = page.getByTestId('end-date-input');
  ...
  test('TC-08A-06: View history record detail', async ({ page }) => {
  ...
  test('TC-08A-07: Reset date functionality', async ({ page }) => {
    const startDateInput = page.getByTestId('start-date-input');
    const endDateInput = page.getByTestId('end-date-input');
