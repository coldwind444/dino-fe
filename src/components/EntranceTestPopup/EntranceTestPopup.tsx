...
export default function EntranceTestPopup({ close }: { close: () => void }) {
    return (
        <AnimatePresence>
            <motion.div
                initial={{ opacity: 0 }}
            ...
            >
                ...
                >
                    {/** Illustration */}
                    <Image src={feather} height={500} width={500} alt="" className="absolute -translate-x-1/3" />
                    {/** Close button */}
                    ...
                    <div className="flex flex-col w-3/4 h-full ml-auto mr-0 gap-10">
                        <div className={clsx(baloo.className, 'flex flex-col font-bold')}>
                            <h1 className="text-amber-500 text-3xl">CHÀO TÂN</h1>
                            <h2 className="text-[#23BEAA] text-xl">Bạn đã sẵn sàng làm bài kiểm tra đầu vào chưa ?</h2>
                            <p className="text-[rgba(0,0,0,0.5)] text-[16px] text-justify text-wrap max-w-[85%] mt-7">
                                Bài kiểm tra đầu vào của Dino Math được biên soạn nhằm mục đích kiểm tra năng lực của học sinh.
                                Kết quả của bài kiểm tra này chỉ được dùng để đánh giá trình độ ban đầu của bạn, ngoài ra không dùng với bất kỳ mục đích nào khác.
                                Vì thế, đừng quá căng thẳng nhé. Chúc bạn làm bài thật tốt !
                            </p>
                        </div>
