...
import { faCaretDown, faHouse, faCubes, faFireFlameCurved, faTrophy, faBarsProgress, faGamepad, faUser } from "@fortawesome/free-solid-svg-icons"
...
export default function Navbar(
    { isAuthenticated = false, username = null, role = null, avatarUrl = '', notifications = [] }:
        {
        ...
        <div className={clsx(
            ...
            }
            {
                !isAuthenticated ?
                    ...
                    :
                    (<div className={clsx('flex items-center justify-center gap-[20px] mr-[10px]')}>
                        <div className={clsx(
                        ...
                        <div className={clsx('flex flex-col justify-center mt-[5px]')}>
                            <label className={clsx(righteous.className, 'select-none')}>Xin chào,</label>
                            <label className={clsx(roboto.className, 'text-[22px] select-none font-bold')}>Tân</label>
                        </div>
                        <FontAwesomeIcon onClick={() => setPopupShow(prev => !prev)} 
                                        className={clsx("cursor-pointer transition-all duration-300", { 'rotate-180' : popupShow})} 
                                        icon={faCaretDown} />
                    </div>)
            }
            {popupShow && 
            <div className={clsx(
                ...
                <div className={clsx(
                    ...
                    <label className={clsx("font-bold cursor-pointer select-none", roboto.className)}>Hồ sơ</label>
                </div>
                <div className={clsx(
                    'flex gap-[20px] items-center pl-[20px] pt-[5px] pb-[5px] cursor-pointer hover:bg-[rgba(0,0,0,0.05)]'
                )}>
                    <FontAwesomeIcon className="text-[#FF5964]" icon={faSignOut}/>
                    <label className={clsx("text-[#FF5964] font-bold cursor-pointer select-none", roboto.className)}>Đăng xuất</label>
                </div>
            </div>}
        </div>
    )
